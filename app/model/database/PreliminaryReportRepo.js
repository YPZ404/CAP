 ...
 */
 ...
 export class PreliminaryReportRepo {
    ...
    async createReport(patientId) {
      const sql = 'INSERT INTO PreliminaryReport (patient_id) VALUES (?);';
     ...
     */
    async getCurrentReport(reportId) {
        const sql = 'SELECT * FROM PreliminaryReport WHERE report_id == ?;';
        const args = [reportId];
     ...
     * @returns 
     */  
    async setMemoryTestResult(reportId, memory_test_result){
      
        const rs = await this.da.runSqlStmt(
        `UPDATE PreliminaryReport SET memory_test_result = ? WHERE report_id== ?;`,
        [reportId, memory_test_result],
        );
        return rs.insertId;
     ...
     * @returns 
     */  
     async setReactionTestResult(reportId, reaction_test_result){
      
        const rs = await this.da.runSqlStmt(
        `UPDATE PreliminaryReport SET reaction_test_result = ? WHERE report_id== ?;`,
        [reportId, reaction_test_result],
        );
        return rs.insertId;
     ...
     * @returns 
     */  
     async setBalanceTestResult(reportId, balance_test_result){
      
        const rs = await this.da.runSqlStmt(
        `INSERT INTO PreliminaryReport (report_id, balance_test_result) VALUES (?, ?);`,
        [reportId, balance_test_result],
        );
        return rs.insertId;
